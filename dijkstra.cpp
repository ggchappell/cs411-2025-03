...
const int INF = -1;                 // As distance, means no path
const size_t NO_VERT = 9999999ULL;  // Means no vertex
...
tuple<vector<Edge>, vector<int>, vector<size_t> > dijkstra(
...
// Main program
// Compute a single-source shortest-path tree using function dijkstra.
// Print graph and result.
int main()
{
    ...
    cout << "Press ENTER to run Dijkstra's Algorithm ";
    userPause();
    ...
    cout << "Running Dijkstra's Algorithm "
         << "(start vertex: " << start << ") ... ";
    cout.flush();
    auto result = dijkstra(adjlists, wmatrix, start);
    ...
    cout << "Edges in single-source shortest-path tree:\n";
    if (tree.size() == 0)
        cout << "[NONE]\n";
